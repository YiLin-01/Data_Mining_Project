...
def gcn_norm(edge_index, edge_flag, edge_attr=None, num_nodes=None, improved=False,
    ...
    if isinstance(edge_index, SparseTensor):
        ...
        num_nodes = maybe_num_nodes(edge_index, num_nodes)  #仍 132

        if edge_attr is None:   #this if 跳过
...
class MPConv(MessagePassing):
    def __init__(self, in_channels, out_channels, improved: bool = False, cached: bool = False,
                 add_self_loops: bool = True, normalize: bool = True, bias: bool = True):        # in_channels = 132, out_channels = 16
        super(MPConv, self).__init__(aggr='add')

        self.improved = improved         #false
        self.cached = cached             #false
        self.add_self_loops = add_self_loops      #true
        self.normalize = normalize          #true
        self._cached_edge_index = None
        self._cached_adj_t = None
        self.__explain__ = False

        self.weight = Parameter(torch.Tensor(in_channels, out_channels))   # weight = Tensor( 132, 16 )

        self.lin = torch.nn.Linear(out_channels*2 + 1, out_channels)     # lin = Linear( 33, 16 )

        if bias:
            self.bias = Parameter(torch.Tensor(out_channels))   # bias = Tensor( 16 )
        else:
...
class IBGConv(torch.nn.Module):
    def __init__(self, input_dim, args, num_classes):   # input_dim = number_features = 132, num_classes = 2
        super(IBGConv, self).__init__()
        self.activation = torch.nn.ReLU()
        self.convs = torch.nn.ModuleList()

        hidden_dim = args.hidden_dim    # hidden_dim = 16
        num_layers = args.n_GNN_layers    # num_layers = 2
        self.pooling = args.pooling      # pooling = sum

        for i in range(num_layers):    # i = 0, 1
            if i == 0:
                conv = MPConv(input_dim, hidden_dim)     # 第 0 层: MPConv ( 132, 16  )
            elif i != num_layers - 1:
                conv = MPConv(hidden_dim, hidden_dim)
            else:
                conv = MPConv(hidden_dim, num_classes)    # 第 1 层：MPConv ( 16, 2 )
...
class MLP(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, activation, n_classes=0):     # 16, 16, 1, relu, 2
        super(MLP, self).__init__()
        self.net = []
        self.net.append(torch.nn.Linear(input_dim, hidden_dim))  #Linear( 16, 16 )
        self.net.append(activation())         # ReLU()
        for _ in range(num_layers - 1):
            self.net.append(torch.nn.Linear(hidden_dim, hidden_dim))
            self.net.append(activation())
        self.net = torch.nn.Sequential(*self.net)
        self.shortcut = torch.nn.Linear(input_dim, hidden_dim)   # shortcut: Linear( 16, 16 )

        if n_classes != 0:
            self.classifier = torch.nn.Linear(hidden_dim, n_classes)       # Classifier: Linear(16, 2)
